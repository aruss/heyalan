...
{
    [Theory]
    [InlineData("https://shelfbuddy.test")]
    [InlineData("https://shelfbuddy.test/")]
    public async Task RegisterWebhookAsync_ComposesWebhookUrlFromAppOptions(string baseUrl)
    {
        ...
        Assert.Equal("https://shelfbuddy.test/webhooks/telegram/12345:token-value", webhookUrl);
    }
    ...
    public async Task RegisterWebhookAsync_SendsSecretTokenAndAllowedUpdates()
    {
        ...
        TelegramService service = CreateService(handler, new Uri("https://shelfbuddy.test"));
