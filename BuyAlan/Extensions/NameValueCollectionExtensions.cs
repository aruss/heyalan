...
{
    public static IDictionary<string, string[]> ToFullDictionary(
        this NameValueCollection source)
    {
        return source.AllKeys
            .ToDictionary(k => k, k => source.GetValues(k));
    }

    public static NameValueCollection FromFullDictionary(
    ...
    public static string ToFormPost(
        this NameValueCollection collection)
    {
        var builder = new StringBuilder(128);
        const string inputFieldFormat =
    ...
    internal static string ConvertFormUrlEncodedSpacesToUrlEncodedSpaces(
        string str)
    {
        ...
        string value)
    {
        ...
        string encodedName = urlEncode ?
            UrlEncoder.Default.Encode(effectiveName) :
            effectiveName;

        string effectiveValue = value ?? String.Empty;

        string encodedValue = urlEncode ?
            UrlEncoder.Default.Encode(effectiveValue) :
            effectiveValue;

        encodedValue =
            ConvertFormUrlEncodedSpacesToUrlEncodedSpaces(encodedValue);
